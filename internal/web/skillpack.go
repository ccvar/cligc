...
func openAPISpec(base, title string) string {
	ops := []apiOp{
		...
		{"post", "/posts", "createDraft", "新建草稿。始终是草稿，不会上线。cover_media_id 设封面", "posts:write"},
		{"patch", "/posts/{id}", "updatePost", "修改文章。cover_media_id 传 0 取掉封面，不传则不动", "posts:write"},
		...
		{"get", "/categories", "listCategories", "列出板块、各语言的名字，以及全站已发布篇数", "posts:read"},
		{"post", "/categories", "createCategory", "新建板块。names/descriptions 按语言给译名，slug 各语言共用", "posts:write"},
		{"patch", "/categories/{id}", "updateCategory", "改板块的名称（含各语言译名）、地址、排序", "posts:write"},
		...
		{"post", "/media", "uploadMedia", "上传图片（multipart/form-data）。自动转 WebP 并按上限缩小，返回的宽高可能和传入的不同；返回的 id 可以拿去当文章封面", "media:write"},
...
- `+"`posts:publish`"+` —— 给了它，AI 就能直接把文章推上线。
  想让 AI 替你发布就勾上，技能包里写清了它该怎么用：只在你明确说"发布"
  时才发，一次一篇，发完报告地址。不想让它发就别勾，它会老实存草稿。
  每日发布上限始终生效，挡的是"一次灌进几十篇"。
